...
import { DataSetNavigationStrategy } from './dataset-navigation-strategy';
...
describe('DataSetNavigationStrategy', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('Should init strategy to empty data and try to display active element', () => {
    const getSortedVisibleDatasetMetasSpy = jest
      .fn()
      .mockImplementation(() => []);
    const strategy = new DataSetNavigationStrategy({
      ...chart,
      getSortedVisibleDatasetMetas: getSortedVisibleDatasetMetasSpy,
    } as unknown as Chart);
  ...
  it('Should hide active element', () => {
    const strategy = new DataSetNavigationStrategy(chart);
    strategy.hide();
