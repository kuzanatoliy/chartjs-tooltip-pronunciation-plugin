...
import { DataFirstNavigationStrategy } from './data-first-navigation-strategy';
...
describe('DataFirstNavigationStrategy', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('Should init strategy to oneset type', () => {
    ...
    new DataFirstNavigationStrategy({
      ...chart,
      getSortedVisibleDatasetMetas: getSortedVisibleDatasetMetasSpy,
    } as unknown as Chart);
    expect(isOnesetChart).toHaveBeenCalled();
  ...
  it('Should init strategy to empty data and try to display active element', () => {
    const getSortedVisibleDatasetMetasSpy = jest
      .fn()
      .mockImplementation(() => []);
    const strategy = new DataFirstNavigationStrategy({
      ...chart,
      getSortedVisibleDatasetMetas: getSortedVisibleDatasetMetasSpy,
    } as unknown as Chart);
  ...
  it('Should hide active element', () => {
    const strategy = new DataFirstNavigationStrategy(chart);
    strategy.hide();
